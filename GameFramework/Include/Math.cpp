...
Vector2 GetRotatePointWithDatum(const Vector2& Datum, const Vector2& Dest, int Radian)
{
	float x = (Dest.x - Datum.x) * cos(Radian) - ((double)Dest.y - Datum.y) * sin(Radian);
	float y = (Dest.x - Datum.x) * sin(Radian) + ((double)Dest.y - Datum.y) * cos(Radian);
