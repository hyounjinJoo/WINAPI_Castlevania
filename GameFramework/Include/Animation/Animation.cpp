...
void CAnimation::ChangeAnimationEnd(const std::string& Name)
{
	if (m_CurrentAnimation->Sequence->GetName() == Name)
		return;
...
bool CAnimation::CheckCurrentAnimationEnd()
{
	bool	AnimEnd = false;
	...
	{
		if (m_CurrentAnimation->Frame == m_CurrentAnimation->Sequence->GetFrameCount() - 1)
			AnimEnd = true;
	}
