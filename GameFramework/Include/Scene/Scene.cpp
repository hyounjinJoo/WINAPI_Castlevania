...
CScene::CScene()
{
	...
	m_RenderEffectCount = 0;
	m_RenderEffectCapacity = 100;
	m_RenderEffectArray = new CGameObject * [m_RenderEffectCapacity];
...
CScene::~CScene()
{
	...
	SAFE_DELETE_ARRAY(m_RenderEffectArray);
...
CGameObject* CScene::FindEffectObject(const std::string& Name)
{
	auto	iter = m_EffectObjList.begin();
	auto	iterEnd = m_EffectObjList.end();
...
bool CScene::PostUpdate(float DeltaTime)
{
	...
	{
		std::list<CSharedPtr<CGameObject>>::iterator	iter = m_EffectObjList.begin();
		std::list<CSharedPtr<CGameObject>>::iterator	iterEnd = m_EffectObjList.end();
...
bool CScene::Render(HDC hDC)
{

	{
		...
		if (m_MapList.size() >= 2)
		{
			qsort(&m_MapList[0], (size_t)m_MapList.size(), sizeof(CMapBase*),
				CScene::SortZOrderMap);
		}
...

	{
		...
		for (; iter != iterEnd;)
		{
			...
			if (!(*iter)->IsCull())
			{
				if (m_RenderEffectCount == m_RenderEffectCapacity)
				{
					m_RenderCapacity *= 2;

					CGameObject** Array = new CGameObject * [m_RenderCapacity];

					memcpy(Array, m_RenderEffectArray, sizeof(CGameObject*) * m_RenderEffectCount);

					delete[]	m_RenderEffectArray;

					m_RenderEffectArray = Array;
				}

				m_RenderEffectArray[m_RenderEffectCount] = *iter;
				++m_RenderEffectCount;
			}
	...
	if (m_RenderEffectCount == m_RenderEffectCapacity)
	{
		m_RenderEffectCapacity *= 2;

		CGameObject** Array = new CGameObject * [m_RenderEffectCapacity];
	...
	qsort(m_RenderEffectArray, (size_t)m_RenderEffectCount, sizeof(CGameObject*),
		CScene::SortY);

	for (int i = 0; i < m_RenderEffectCount; ++i)
	{
		m_RenderEffectArray[i]->Render(hDC);
	}
...
CMapBase* CScene::FindMap(const std::string& Name)
{
	auto	iter = m_MapList.begin();
	auto	iterEnd = m_MapList.end();
...
void CScene::DeleteMap(const std::string& Name)
{
	auto	iter = m_MapList.begin();
	auto	iterEnd = m_MapList.end();
